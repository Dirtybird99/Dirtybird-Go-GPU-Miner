...
// DERO target semantics: a hash passes iff its byte-reversed value, read as a
// big-endian integer, is <= (1<<256)/difficulty. The big.Int division happens
// once per job (ComputeTarget); the per-hash check (MeetsTarget) is an
// alloc-free byte compare.
