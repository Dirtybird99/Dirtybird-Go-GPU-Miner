...
func (c *Client) connectAndServe(ctx context.Context) (jobs uint64) {
	...
	// Bound incoming frame size: a job is <200 B, so 64 KiB is generous. Without
	// this a hostile or MITM peer (TLS verification is off) could stream an
	// unbounded frame and OOM the miner. gorilla returns an error past the limit,
	// which the reader treats as a dropped connection and redials.
	...
	// safe alongside it). connDone lets the reader release the writer when the
	// connection dies with no share queued — otherwise the writer stays parked
	// in this select (conn.Close alone does not unblock it), the reader blocks
	// on writerDone, and connectAndServe never returns to redial.
	...
	for {
		...
		if err := conn.ReadJSON(&j); err != nil {
			...
			conn.Close()
			close(connDone) // release the writer so connectAndServe can redial
			<-writerDone
			return jobs
		}
		jobs++
